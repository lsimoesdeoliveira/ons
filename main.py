...
def menu():
    ...
    while True:
        selection = input('Escolha os dados a serem atualizados (UHE ou UTE ou EOL ou ALL): ').upper()
        if selection == 'ALL':
            files = [menu['UHE'][0], menu['UTE'][0], menu['EOL'][0]]
            complement = [menu['UHE'][1], menu['UTE'][1], menu['EOL'][1]]
            break
        elif selection in menu.keys():
            files = menu[selection][0]
            complement = menu[selection][1]
            break
        else:
            continue
    return files, complement


files, complement = menu()
...
for arquivo in files:
    ...
    print('A última data de medições no arquivo {} é: {}'.format(arquivo, d1))
